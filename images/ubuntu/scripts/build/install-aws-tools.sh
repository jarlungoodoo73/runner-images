...
# Detect system architecture and set appropriate URLs
architecture=$(dpkg --print-architecture)
if [[ "$architecture" == "arm64" ]]; then
    awscli_arch="aarch64"
    smplugin_arch="ubuntu_arm64"
    sam_cli_arch="arm64"
elif [[ "$architecture" == "amd64" ]]; then
    awscli_arch="x86_64"
    smplugin_arch="ubuntu_64bit"
    sam_cli_arch="x86_64"
else
...
awscliv2_archive_path=$(download_with_retry "https://awscli.amazonaws.com/awscli-exe-linux-${awscli_arch}.zip")
unzip -qq "$awscliv2_archive_path" -d /tmp
/tmp/aws/install -i /usr/local/aws-cli -b /usr/local/bin

smplugin_deb_path=$(download_with_retry "https://s3.amazonaws.com/session-manager-downloads/plugin/latest/${smplugin_arch}/session-manager-plugin.deb")
apt-get install "$smplugin_deb_path"

# Download the latest aws sam cli release
aws_sam_cli_archive_name="aws-sam-cli-linux-${sam_cli_arch}.zip"
